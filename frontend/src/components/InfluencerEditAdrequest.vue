    ...
    methods: {
      ...
      async updateAdRequest() {
        ...
        try {
          ...
          setTimeout(() => {
            this.successMessage = '';
          }, 7000); // Clear success message after 2 seconds
        } catch (error) {
          this.errorMessage = error.message;
          setTimeout(() => {
            this.errorMessage = '';
          }, 7000); // Clear error message after 2 seconds
        }
      },
      goBack() {
        this.$router.push('/influencer_dashboard'); // Redirect to the influencer dashboard or relevant page
      },
    },
  };
  </script>
