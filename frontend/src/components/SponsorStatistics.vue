<template>
    <div class="sponsor-stats">
      ...
      <div v-if="stats" class="stats-container">
        ...
        <div class="stats-item">
          <div class="stat-label">Total Campaigns Created:</div>
        ...
        <div class="stats-item">
          <div class="stat-label">Total Ad Requests Accepted:</div>
          <div class="stat-value">{{ stats.total_adrequest_accepted }}</div>
        </div>
  
        <div class="stats-item">
          <div class="stat-label">Total Ad Requests Declined:</div>
  ...
  export default {
    ...
    mounted() {
      this.fetchSponsorStatistics();
    },
    methods: {
      async fetchSponsorStatistics() {
        const token = localStorage.getItem('authToken');
        const sponsor_id = localStorage.getItem('sponsor_id');
  
        if (!this.sponsorId) {
          this.errorMessage = 'Sponsor ID not provided.';
          return;
        }
  
        try {
          const response = await fetch(`http://127.0.0.1:5000/sponsor/${sponsor_id}/statistics`, {
            method: 'GET',
            headers: {
          ...
          if (!response.ok) {
            throw new Error(data.message || 'Failed to fetch sponsor statistics');
          }
